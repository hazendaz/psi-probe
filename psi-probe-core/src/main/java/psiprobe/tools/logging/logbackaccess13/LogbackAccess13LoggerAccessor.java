 ...
 */
public class LogbackAccess13LoggerAccessor extends DefaultAccessor {
   ...
   */
  public LogbackAccess13AppenderAccessor getAppender(String name) {
    try {
      Object appender = MethodUtils.invokeMethod(getTarget(), "getAppender", name);
      if (appender == null) {
        List<LogbackAccess13AppenderAccessor> appenders = getAppenders();
        for (LogbackAccess13AppenderAccessor wrappedAppender : appenders) {
   ...
   */
  private LogbackAccess13AppenderAccessor wrapAppender(Object appender) {
    try {
      if (appender == null) {
        throw new IllegalArgumentException("appender is null");
      }
      LogbackAccess13AppenderAccessor appenderAccessor = new LogbackAccess13AppenderAccessor();
