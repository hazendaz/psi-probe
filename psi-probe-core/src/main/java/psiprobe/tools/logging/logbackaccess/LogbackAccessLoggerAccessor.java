 ...
 */
public class LogbackAccessLoggerAccessor extends DefaultAccessor {
   ...
   */
  public LogbackAccessAppenderAccessor getAppender(String name) {
    try {
      Object appender = MethodUtils.invokeMethod(getTarget(), "getAppender", name);
      if (appender == null) {
        List<LogbackAccessAppenderAccessor> appenders = getAppenders();
        for (LogbackAccessAppenderAccessor wrappedAppender : appenders) {
   ...
   */
  private void wrapAndAddAppender(Object appender, List<LogbackAccessAppenderAccessor> appenders) {
    LogbackAccessAppenderAccessor appenderAccessor = wrapAppender(appender);
   ...
   */
  private LogbackAccessAppenderAccessor wrapAppender(Object appender) {
    try {
      if (appender == null) {
        throw new IllegalArgumentException("appender is null");
      }
      LogbackAccessAppenderAccessor appenderAccessor = new LogbackAccessAppenderAccessor();
